...
! ***********************************************************************
      ...
      ! Called from run_star_extras self-defined subroutine adipls_calculate_frequencies 
      subroutine adipls_get_one_el_info( &
            s, l, iem, nu1, nu2, iscan, nsel, dels, itrsig, nsig, R, G, M, &    ! iem is used for rotation controls only (?)
	    irotsl, nsem, idems, iper, ivarf, irotkr, nprtkr, ispcpr, omega0, &  ! passed as arguments from run_star_extras 
            add_center_point, keep_surface_point, add_atmosphere, &
            redist_mesh, store_for_adipls, &
            save_mode_info, save_mode_filename_in, &
            num, l_order, l_em, l_freq, l_inertia, l_beta, l_split1, l_split2,  &
            l_split3, l_splitted_freq, ierr)  

         use num_lib, only: qsort
         use star_def, only: star_info
         type (star_info), pointer :: s

         integer, intent(in) :: l, iscan, nsel, itrsig, nsig, iper, ivarf,  &
	    iem, irotsl, nsem, idems, irotkr, nprtkr, ispcpr 
         real(dp), intent(in) :: nu1, nu2, R, G, M, dels, omega0
         logical, intent(in) :: &
            add_center_point, keep_surface_point, add_atmosphere, &
            redist_mesh, store_for_adipls, save_mode_info
         character (len=*), intent(in) :: save_mode_filename_in
         integer, intent(out) :: num, ierr 
         real(dp), pointer, dimension(:) :: l_freq, l_inertia, l_beta, l_split1, l_split2, l_split3, l_splitted_freq  ! outputs 
         integer, pointer, dimension(:) :: l_order , l_em  
         real(dp) :: el, sig_fac, els1, sig1, sig2, dfsig
         integer :: k, i, j, igm1kr, npgmkr
         integer, pointer :: index(:) 
         
         logical, parameter :: dbg = .true.  !.false.

         include 'formats'

         ierr = 0
	 el = dble(l)  ! replace l by el, real(dp), passed to set_adipls_controls 
         sig_fac = (2*pi)**2*R**3/(G*M)
!         nsel = 0
!         dels = 1
         els1 = dble(l) ! double precision real
!         itrsig = 1
...
!	 em = -1  ! might have to be removed; for now treat as integer input to be passed to adipls
!	 irotsl = 1 
!	 nsem = -1
!	 dems = 1 
!	 iper = 1
!	 ivarf = 2
!	 irotkr = 21  ! turning on second-order rotation
!	 nprtkr = 70  ! print rotational kernel at 70 points
!	 omgrtp(1) = 5.0  pull rotation rate settings out

         
	 ! store parameters into common blocks to be passed to adipls 
	 ! should parameter values be defined before or after the commons?
         if (dbg) write(*,*) 'call set_adipls_controls'
         call set_adipls_controls( &
            el, nsel, els1, dels, itrsig, iscan, sig1, sig2, dfsig, nsig,  & 
	    iem, irotsl, nsem, idems, iper, ivarf, omega0,  &
            irotkr, nprtkr, igm1kr, npgmkr, ispcpr)
	 write(*,*) 'Show me irotkr, irotsl, iem, omega0, ispcpr after call set_adipls_controls' 
	 write(*,*) irotkr, irotsl, iem, omega0, ispcpr
         if (dbg) write(*,*) 'done set_adipls_controls'

         save_mode_filename = save_mode_filename_in

	 el_to_save = l  ! for astero_data
	 em_to_save = iem ! try to do it similar to l, parameter originating from astero_data
         ! so that it becomes a single value as the input 
       
         num_results = 0

         if (dbg) write(*,*) 'call run_adipls'
	 write(*,*) 'Show me irotkr, irotsl, iem, omega0 after calling run_adipls'
	 write(*,*) irotkr, irotsl, iem, omega0 

         call run_adipls(s, .false., store_for_adipls, &
            add_center_point, keep_surface_point, add_atmosphere, &
            redist_mesh, ierr)

      !  back to here after calling dump_obs in adipls
  ...
  !  Segmentation fault invalid memory reference error probably originates from here

         ! sort results by increasing frequency, with corresponding order, iem and inertia 
	 ! all four of them are default outputs from adipls, can iem be changed as input argument?
         ...
         if (.not. associated(l_order)) then
            write(*,*) 'allocating l_order'
            allocate(l_order(num_results))
         else if (num_results >= size(l_order,dim=1)) then ! enlarge
            write(*,*) 'calling realloc_integer'
            call realloc_integer(l_order,num_results,ierr)
            if (ierr /= 0) return
         end if

         if (.not. associated(l_em)) then     ! might have to be removed
            write(*,*) 'allocating l_em'
         ...
         if (.not. associated(l_beta)) then        ! added by me 
            write(*,*) 'allocating l_beta'
            allocate(l_beta(num_results))
         else if (num_results >= size(l_beta,dim=1)) then ! enlarge   ! only this condition was saisfied, not the first line
            write(*,*) 'calling realloc_double'
            call realloc_double(l_beta,num_results,ierr)
            if (ierr /= 0) return
         end if

         if (.not. associated(l_split1)) then        ! added by me 
            write(*,*) 'allocating l_split1'
            allocate(l_split1(num_results))
         else if (num_results >= size(l_split1,dim=1)) then ! enlarge
            write(*,*) 'calling realloc_double'
            call realloc_double(l_split1,num_results,ierr)
            if (ierr /= 0) return
         end if

         if (.not. associated(l_split2)) then        ! added by me 
            write(*,*) 'allocating l_split2'
            allocate(l_split2(num_results))
         else if (num_results >= size(l_split2,dim=1)) then ! enlarge
            write(*,*) 'calling realloc_double'
            call realloc_double(l_split2,num_results,ierr)
            if (ierr /= 0) return
         end if

         if (.not. associated(l_split3)) then        ! added by me 
            write(*,*) 'allocating l_split3'
            allocate(l_split3(num_results))
         else if (num_results >= size(l_split3,dim=1)) then ! enlarge
            write(*,*) 'calling realloc_double'
            call realloc_double(l_split3,num_results,ierr)
            if (ierr /= 0) return
         end if
         
        if (.not. associated(l_splitted_freq)) then        ! added by me 
            write(*,*) 'allocating l_splitted_freq'
            allocate(l_splitted_freq(num_results))
         else if (num_results >= size(l_splitted_freq,dim=1)) then ! enlarge
            write(*,*) 'calling realloc_double'
            call realloc_double(l_splitted_freq,num_results,ierr)
            if (ierr /= 0) return
         end if
         ...
         do j = 1, num_results
            i = index(j)
            l_order(j) = order(i)
            l_em(j) = em(i)  ! l_em as an output argument of subroutine run_adipls (or not)
            l_freq(j) = cyclic_freq(i)
            l_inertia(j) = inertia(i)
	    l_beta(j) = beta(i)          ! added by me 
	    l_split1(j) = split1(i)
	    l_split2(j) = split2(i)
	    l_split3(j) = split3(i)
	    l_splitted_freq(j) = splitted_freq(i)
      ...
      ! saving results to save_mode.data (similar to gyre_support) instead of just saving one mode to eigen.data 
     
      subroutine adipls_mode_info( &
            l, order, em, freq, inertia, beta, split1, split2, split3, splitted_freq,  &      ! added by me 
	    x, y, aa, data, nn, iy, iaa, ispcpr)
      ! arguments el(n) passed to here as l, etc. 

         integer, intent(in) :: l, order, em
         real(dp), intent(in) :: freq, inertia, beta, split1, split2, split3, splitted_freq
         ...
         write(*,*) 'Subtoutine adipls_mode_info called'
         ...
         write(*,*) 'Show me splitted_freq in adipls_support adipls_mode_info'
         write(*,*) splitted_freq
         ...
         write(*,'(3a8,99a17)') 'l', 'order', 'm', 'freq (uHz)', 'inertia',  &
	    'beta', 'split1', 'split2', 'split3', 'split_freq (uHz)'

      !  em already converted to integer
         write(*,'(3i8.1,f17.5,1pe17.5,0pf17.6,1pe17.5,1pe17.5,1pe17.5,1pe17.5)') l, order, em,  &
	    freq, inertia, beta, split1, split2, split3, splitted_freq

         ierr = 0
         iounit = alloc_iounit(ierr)
         if (ierr /= 0) return

         open(unit=iounit, file=trim(save_mode_filename), action='write', iostat=ierr, position='append')
         ...
         write(iounit,'(3i8.1,f17.5,1pe17.5,0pf17.6,1pe17.5,1pe17.5,1pe17.5,1pe17.5)') l, order, em, & 
	    freq, inertia, beta, split1, split2, split3, splitted_freq !, nn-skip
...
!         end do
      ...
      subroutine run_adipls( &
         ...
         common/cstdio/ istdin, istdou, istdpr, istder  ! standard input and output 
         integer :: istdin, istdou, istdpr, istder
	 common/rotdat/ iem, irotsl, nsem, idems   ! added by me, just to declare the write line 
	 integer :: iem, irotsl, nsem, idems
	 common/rotvel/ omega0
	 real(dp) :: omega0
	 common/coutpt/ nout, nprcen, iper, irotkr, nprtkr,  &
	    igm1kr, npgmkr, nfmode, nfmesh, ispcpr, npout,  &
	    nobs_stmx, nfmscn
	 integer :: nout, nprcen, iper, irotkr, nprtkr,  &   ! added by me
	    igm1kr, npgmkr, nfmode, nfmesh, ispcpr, npout,  &
	    nobs_stmx, nfmscn
	 ...
	 write(*,*) 'Show me irotkr, irotsl, iem, omega0 before call adipls in run_adipls'
	 write(*,*) irotkr, irotsl, iem, omega0

	 ! call adipls main subroutine 
	    ...
	    write(*,*) 'Show me irotkr, irotsl and iem before call adipls in setup_adipls'
	    write(*,*) irotkr, irotsl, iem 
    ...
    ! set parameters in common blocks to be passed to adipls 

      subroutine set_adipls_controls( &
            el, nsel, els1, dels, itrsig, iscan, sig1, sig2, dfsig, nsig,  & 
	    iem, irotsl, nsem, idems, iper, ivarf, omega0,  &
            irotkr, nprtkr, igm1kr, npgmkr, ispcpr)
         integer, intent(in) :: nsel, itrsig, iscan, nsig, &
	    iem, irotsl, nsem, idems, iper, ivarf,  &
            irotkr, nprtkr, igm1kr, npgmkr, ispcpr
         real(dp), intent(in) :: el, els1, dels, sig1, sig2, dfsig, omega0

         !  common blocks for parameter transmission (to be passed on to adipls.c.d.f)
         common/cadr_param/ &
	    ...
	    para_el, para_els1, para_dels, &
         ...
         common/cadi_param/ &
            ...
            ipara_istsig, ipara_inomd1, ipara_iscan  ! put irotkr in common/coutput/ 
         integer :: &
            ipara_nsel, ipara_nsig1, ipara_nsig2, ipara_itrsig, ipara_nsig, &
            ipara_istsig, ipara_inomd1, ipara_iscan 

	 ! l_em in not an input, only pass the input values in the common blocks (?)         
         common/rotdat/  &
            ipara_iem, ipara_irotsl, ipara_nsem, ipara_idems ! nsem, ems1, dems not in adipls.c.d.f commons list 
	 integer :: &
	    ipara_iem, ipara_irotsl, ipara_nsem, ipara_idems

         ! common for rotation angular velocity, added by me 
	 common/rotvel/  &
	    para_omega0     ! para_ for real
	 real(dp) ::   & 
	    para_omega0  
	 ...
	 ! Storing parameters into common blocks 
	 ! List of arguments in subroutine set_adipls_controls are passed to adipls 
	 ! not every single default variable in the common block  
         para_el = dble(el)
         ipara_nsel = nsel
         para_els1 = els1
         para_dels = dels
         ...
         para_dfsig = dfsig
         ipara_nsig = nsig
	 ipara_iem = iem  !!! might have to be removed otherwise conflicts with output l_em
	 ipara_irotsl = irotsl
	 ipara_nsem = nsem 
	 ipara_idems = idems
	 para_omega0 = omega0  ! added by me 
	 ipara_iper = iper 
	 ipara_ivarf = ivarf 
         ipara_irotkr = irotkr
         ipara_nprtkr = nprtkr
	 ...
	 write(*,*) 'Show me irotsl and em stored in common/rotdat/'
	 write(*,*) ipara_irotsl, ipara_iem 
	 write(*,*) 'Show me irotkr, ispcpr stored in common/coutpt/'
	 write(*,*) ipara_irotkr, ipara_ispcpr
