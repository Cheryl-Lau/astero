...
! ***********************************************************************
      ...
      subroutine get_one_el_info( &
         ...
         integer :: l,iscan, i1, i2, nsel, itrsig, nsig, iper, ivarf,  &
	    ll_em, irotsl, nsem, dems, irotkr, nprtkr, ispcpr  ! no intent(in), not called from outside subroutines so arg not defined  
         real(dp) :: ll, nu1, nu2, R, G, M, omega0          ! same 
       ! ll=l, l (integer) is passed to gyre, ll (real) is passed to adipls

         logical, intent(in) :: store_model
         character (len=*), intent(in) :: code
         integer, intent(out) :: ierr
         real(dp) :: nu_obs, dist_j, nu, dist, min_dist, min_freq, &
            sig_fac, b, sum_1, sum_2, sum_3, empty(0), els1, dels, sig1, sig2, dfsig
         integer :: min_dist_j, min_order, n, cnt, int_empty(0), &
            igm1kr, npgmkr, num_l0_terms, k, i, j
         ...
         write(*,*) 'astero_support get_one_el_info called'
	    ...
	    el_to_save = ll  ! rename it as l instead of el to avoid symbol conflict with astero_data 
	    em_to_save = ll_em ! similar to l, use ll_em instead of em.  *_to_save parameter originating from astero_data
            nsel = 0
            dels = 1
            els1 = dble(l)
            itrsig = 1
...
!               adipls_irotkr, adipls_nprtkr, adipls_igm1kr, adipls_npgmkr)
            ...
            call set_adipls_controls( &
                 ll, nsel, els1, dels, itrsig, iscan, sig1, sig2, dfsig, nsig,  & 
	         ll_em, irotsl, nsem, dems, iper, ivarf, omega0,  &
...
!	       l0_beta, empty, empty, &            ! added by me 
!	       l0_split1, empty, empty, &
!	       l0_split2, empty, empty, &
!	       l0_split3, empty, empty, ierr)
         else if (l == 1) then
...
!	       l1_beta, l1_beta_alt_up, l1_beta_alt_down, &
!	       l1_split1, l1_split1_alt_up, l1_split1_alt_down, &
!	       l1_split2, l1_split2_alt_up, l1_split2_alt_down, &
!	       l1_split3, l1_split3_alt_up, l1_split3_alt_down, ierr)
         else if (l == 2) then
...
!	       l2_beta, l2_beta_alt_up, l2_beta_alt_down, &
!	       l2_split1, l2_split1_alt_up, l2_split1_alt_down, &
!	       l2_split2, l2_split2_alt_up, l2_split2_alt_down, &
!	       l2_split3, l2_split3_alt_up, l2_split3_alt_down, ierr)
         else if (l == 3) then
...
!	       l3_beta, l3_beta_alt_up, l3_beta_alt_down, &
!	       l3_split1, l3_split1_alt_up, l3_split1_alt_down, &
!	       l3_split2, l3_split2_alt_up, l3_split2_alt_down, &
!	       l3_split3, l3_split3_alt_up, l3_split3_alt_down, ierr)
         else
         ...
         subroutine set_to_closest( &         
               ...
               l_order, l_order_up, l_order_down, ierr)
!	       l_beta, l_beta_up, l_beta_down, &           ! added by me 
!	       l_split1, l_split1_up, l_split1_down, &
!	       l_split2, l_split2_up, l_split2_down, &
!	       l_split3, l_split3_up, l_split3_down, ierr)

            real(dp), intent(in) :: l_obs(:)
            real(dp), intent(inout), dimension(:) :: &
               l_freq, l_freq_up, l_freq_down, &
               l_inertia, l_inertia_up, l_inertia_down
!	       l_beta, l_beta_up, l_beta_down, &
!	       l_split1, l_split1_up, l_split1_down, &
!	       l_split2, l_split2_up, l_split2_down, &
!	       l_split3, l_split3_up, l_split3_down
...
!			l_beta_down(i) = beta(j_alt)
!			l_split1_down(i) = split1(j_alt)
!			l_split2_down(i) = split2(j_alt)
!			l_split3_down(i) = split3(j_alt)
                     end if
...
!			l_beta_up(i) = beta(j_alt)
!			l_split1_up(i) = split1(j_alt)
!			l_split2_up(i) = split2(j_alt)
!			l_split3_up(i) = split3(j_alt)
                     end if
                  end if
                  jprev = j
               end if
      ...
      subroutine get_cubic_radial_freq_corr(a3, &
        ...
        l0_freq_corr = l0_freq + correction_factor*a3*l0_freq**3/l0_inertia
      ...
      subroutine get_cubic_all_freq_corr(a3, radial_only, &
        ...
        l0_freq_corr = l0_freq + correction_factor*a3*l0_freq**3/l0_inertia
        l1_freq_corr = l1_freq + correction_factor*a3*l1_freq**3/l1_inertia
        l2_freq_corr = l2_freq + correction_factor*a3*l2_freq**3/l2_inertia
        l3_freq_corr = l3_freq + correction_factor*a3*l3_freq**3/l3_inertia
      ...
      subroutine get_combined_radial_freq_corr(a3, a1, &
        ...
        l0_freq_corr = l0_freq + &
             correction_factor*(a1*l0_freq**(-1)+a3*l0_freq**3)/l0_inertia
      ...
      subroutine get_combined_all_freq_corr(a3, a1, radial_only, &
        ...
        l0_freq_corr = l0_freq + &
             correction_factor*(a1*l0_freq**(-1)+a3*l0_freq**3)/l0_inertia
        l1_freq_corr = l1_freq + &
             correction_factor*(a1*l1_freq**(-1)+a3*l1_freq**3)/l1_inertia
        l2_freq_corr = l2_freq + &
             correction_factor*(a1*l2_freq**(-1)+a3*l2_freq**3)/l2_inertia
        l3_freq_corr = l3_freq + &
             correction_factor*(a1*l3_freq**(-1)+a3*l3_freq**3)/l3_inertia
      ...
      real(dp) function power_law(freq, freq_ref, a, b)
        ...
        power_law = a*pow_cr(freq/freq_ref, b)
      ...
      real(dp) function dpower_law_da(freq, freq_ref, a, b)
        ...
        dpower_law_da = pow_cr(freq/freq_ref, b)
      ...
      real(dp) function dpower_law_db(freq, freq_ref, a, b)
        ...
        dpower_law_db = a*pow_cr(freq/freq_ref, b)*log_cr(freq/freq_ref)
      ...
      real(dp) function sonoi(freq, freq_ref, a, b)
        ...
        sonoi = a*freq_ref*(1d0 - 1d0/(1d0+pow_cr(freq/freq_ref,b)))
      end function sonoi
      ...
      real(dp) function dsonoi_da(freq, freq_ref, a, b)
        ...
        dsonoi_da = freq_ref*(1d0 - 1d0/(1d0+pow_cr(freq/freq_ref,b)))
      ...
      real(dp) function dsonoi_db(freq, freq_ref, a, b)
        ...
        dsonoi_db = a*freq_ref*pow_cr(freq/freq_ref,b)*log_cr(freq/freq_ref)/(1d0+pow_cr(freq/freq_ref,b))**2d0
