! ***********************************************************************
!
!   Copyright (C) 2013-2017  Bill Paxton, Rich Townsend
!
...
! for adipls, not required in gyre, just to initialize the variables in store_oscillation_results call
    new_beta = 0.00    
    new_split1 = 0.00
    new_split2 = 0.00
    new_split3 = 0.00
    new_splitted_freq = 0.00
...
!  arg edited according to new astero_data, but splittings only comes from adipls, no need to edit gyre
...
!    edit this line, only return for wrong degrees, not for wrong orders
!    compute fequencies of ALL orders 
    if (new_el /= el_to_save) return
...
!    append the file rather than writing a new one
    open(unit=iounit, file=trim(save_mode_filename), action='write', iostat=ierr, position='append')
    if (ierr /= 0) return

!    only write the numbers, not the file header
