...
! ***********************************************************************
      ...
      integer :: time0, time1, clock_rate, profile_counter
      double precision, parameter :: expected_runtime = 5.0 ! minutes
...
!          if (ierr /= 0) return
      ...
      subroutine data_for_extra_history_columns(id, id_extra, n, names, vals, ierr)
         ...
         type (star_info), pointer :: s
         integer :: k
         double precision :: integral, lval
         double precision, allocatable :: brunt_N(:)
         integer, parameter:: b8 = selected_real_kind(14)
         real(b8), parameter :: pi = 3.141592653589793239_b8
         
         !! This routine computes the expected asymptotic spacing dP for dipole (l=1) modes [unit = days]
         ! dP = 86400 * (2 * pi^2) / (sqrt{l*(l+1)} * int{(N/r)*dr})
         ...
         do k = 1, s%nz
           brunt_N(k) = (86400./(2*pi))*sqrt(max(0.d0,s% brunt_N2(k)))
!            if(brunt_N(k) < 1.d-2) then
!              brunt_N(k) = 0.d0
!            end if
         end do
         ...
         ! compute the asymptotic spacing dP for dipole (l=1) modes [unit = days]
         lval = 1.d0
         
         names(1) = "dP_dipole"
         vals(1)  = pi / (sqrt(lval*(lval+1.d0))*integral)
         
         deallocate(brunt_N)
         
         
         !! This routine computes the break-up velocity (in rad/s) following the Roche lobe criterion
         ! Omega_crit = sqrt(8 * G * Mstar / (27 * Rstar^3))
         
         names(2) = "Omega_crit"
         vals(2) = sqrt(8.d0 * standard_cgrav * msol * s% star_mass / (27.d0 * (rsol * s% photosphere_r)**3.d0))
